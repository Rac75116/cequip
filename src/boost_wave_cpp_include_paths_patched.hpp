...
class include_paths {
   ...
   public:
    ...
    bool add_include_path(char const *path_, bool is_system = false) {
        return add_include_path(
            path_, (is_system || was_sys_include_path) ? system_include_paths : user_include_paths);
    }
   ...
   protected:
    bool find_include_file(std::string &s, std::string &dir, include_list_type const &pathes,
                           char const *) const;
   ...
   private:
    ...
    void load(Archive &ar, const unsigned int loaded_version) {
        ...
        if (user_paths != user_include_paths) {
            BOOST_WAVE_THROW(preprocess_exception, incompatible_config, "user include paths",
                             load_filepos());
            return;
        }
        if (system_paths != system_include_paths) {
            BOOST_WAVE_THROW(preprocess_exception, incompatible_config, "system include paths",
                             load_filepos());
            return;
        }
...
inline bool include_paths::find_include_file(std::string &s, std::string &dir,
    ...
    for (/**/; it != include_paths_end; ++it) {
        ...
        if (fs::is_regular_file(currpath)) {
...
inline bool include_paths::find_include_file(std::string &s, std::string &dir, bool is_system,
                                             char const *current_file) const {
    namespace fs = boost::filesystem;
    ...
    if (!is_system) {
        if (!was_sys_include_path) {  // set_sys_include_delimiter() not called
            ...
            if (fs::is_regular_file(currpath) && 0 == current_file) {
...
inline bool as_relative_to(boost::filesystem::path const &path, boost::filesystem::path const &base,
    ...
    if (path.has_root_path()) {
        ...
        if (base.has_root_path()) {
            ...
            for (/**/; base_it != base.end(); ++base_it) result /= "..";

            for (/**/; path_it != path.end(); ++path_it) result /= *path_it;
        }
    }
    return true;
}
...
inline void save(
    ...
    boost::serialization::stl::save_collection<
        Archive, typename boost::wave::util::bidirectional_map<std::string, std::string>::type>(ar,
                                                                                                t);
}
...
inline void load(Archive &ar,
    ...
    boost::serialization::stl::load_collection<
        Archive, map_type, boost::serialization::stl::archive_input_unique<Archive, map_type>,
...
inline void serialize(
    Archive &ar, typename boost::wave::util::bidirectional_map<std::string, std::string>::type &t,
    const unsigned int file_version) {
